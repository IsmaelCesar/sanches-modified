...
do
    ...
    do
        ...
        python -m experiments_genetic --results-dir results/genetic/distributions/laplace_${cx_op}_${mut_op} --num-qubits $num_qubits --eps 0.05  --state-type laplace   --state-params x_points="(0, 1)" loc=0.5 scale=0.2 --run-idx 1 --n-gen $n_gen --pop-size $pop_size --crossover-type $cx_op --mutation-type $mut_op --device $device
    done
done
